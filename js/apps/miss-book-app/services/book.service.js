...
function waitQuery(filterBy = null) {
    ...
    if (!filterBy) {
        return new Promise((resolve) => {
            setTimeout(resolve, 1500, books);
        });
    }
    const filteredBooks = _getFilteredBooks(books, filterBy)
    return new Promise((resolve) => {
        setTimeout(resolve, 1500, filteredBooks);
    });
}
...
function addReview(book, review) {
    ...
    else book.reviews = [...book.reviews, review];

    var books = _loadBooksFromStorage()
    const bookIdx = books.findIndex(currBook => book.id === currBook.id)
    books[bookIdx] = book;
...
function getBookById(bookId) {
    const books = _loadBooksFromStorage()
    // return new Promise.Resolve(books.find(book => book.id === bookId));
    return new Promise((resolve) => {
...
function searchBooks(bookName) {
    ...
    const searchStr = bookName.replace(' ', '%20')
    return fetch(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchStr}`)
...
function _createBooks() {
    var books = _loadBooksFromStorage(KEY);
